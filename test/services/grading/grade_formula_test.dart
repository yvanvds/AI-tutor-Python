...
// milestone's expected level, `d` counts only among mastered LOs, the
// proposal is the mastery score itself (P = M, v1.0.16, #191), and mastery is the one-way `firstMasteredAt` stamp, not the
// live belief (#168).
...
void main() {
  group('bijlage B (v1 placeholders: C(k)=k, w_u=0.6, w_d=0.4; P = M)', () {
    test('student A: 8/8 core, 2/3 extension, 4 of 10 at hard → 78', () {
      ...
      expect(roundedProposal(proposalScore(mEnd: m.m)), 78);
    });

    test('student B: 7/8 core, 1/3 extension, nothing at hard → 53', () {
      final los = _los(core: 8, extension: 3);
      final inputs = <String, LoGradeInput>{};
      for (var i = 0; i < 7; i++) {
      ...
      expect(roundedProposal(proposalScore(mEnd: m.m)), 53);
    });
  });
  ...
  group('the proposal (§2.6)', () {
    test('P = M: no growth term, no period-start baseline (#191)', () {
      for (final m in [0.0, 37.5, 52.5, 78.0, 100.0]) {
        expect(proposalScore(mEnd: m), m);
      }
      // v1.0.17 (#187) changed when evidence is asked for, not the formula.
      expect(GradingConstants.formulaVersion, '1.0.17');
    });
