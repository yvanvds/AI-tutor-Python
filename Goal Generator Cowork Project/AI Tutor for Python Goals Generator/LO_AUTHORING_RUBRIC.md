...
teachingTips are read by the runtime LLM, not by the teacher and not by
the student. Write them as **imperatives addressed to the LLM**, not as
third-person observations about students.

- Good: *"Probe whether the student treats unquoted text as a name vs. a
  string literal."*
- Good: *"Lean on the `'score: ' + 5` TypeError as a probe for
  conflation of concatenation with addition."*
- Bad: *"Students often confuse strings with names when they forget
  quotes."* (Describes a phenomenon; doesn't tell the LLM what to do
  about it.)

The rewrite is mechanical: turn *"students often X"* into *"probe
whether…"*, *"watch for…"*, *"lean on… as a probe"*, *"contrast X with
Y when…"*, *"prefer X over Y for first probes"*, *"avoid X"*. If a draft
tip reads like teacher-to-teacher commentary, it's wrong-shaped — the
runtime LLM doesn't need to be informed about students, it needs to be
directed.

**English is preferred** for teachingTips. The runtime LLM follows
English imperatives more reliably than Dutch ones, and these strings
are never shown to students. (Curriculum content — titles, descriptions,
LO statements — stays Dutch.)
