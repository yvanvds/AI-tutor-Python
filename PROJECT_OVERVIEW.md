...
History is managed client-side with three parallel logs capped at 50 entries each: `_allHistory` (across sessions), `_sessionHistory` and `_exerciseHistory` (the current question chain; both restart on a `newSession` call). Each `sendRequest` chooses one of `includeAll | includeSession | exercise | newSession` (#184): question generation goes out on `newSession` with a `recent_questions` block (the last 12 questions, kept by `RecentQuestions` in the tutor service), grading/hint/dialogue calls on `exercise`, the status report on `includeAll`. `instructions` is passed as a system message on every call; the user-turn content is a JSON string built by `QuestionFormatter`.
...
**`turn_history`** *(new)* — append-only audit trail; one doc per graded turn (CONDUCTOR_POLICY §8.1). Doc id = ISO timestamp + random suffix; partition key `uid`. Model: [services/student_state/turn_record.dart](lib/services/student_state/turn_record.dart). Consumed by the debug dialog and the teacher drawer's Signal Events section. Each doc captures: targets (`subgoalId`, `targetLoIds`, `selectionReason`), question (`type`, `difficulty`), result (`quality`, `signals[]` of `TurnLoSignal`, `appliedDeltas[]` of `TurnAppliedSignal`, `loStatus[]` of `TurnLoStatus`), calibration before/after, subgoal progress before/after, plus `events[]` of `TurnSignalEvent`. Each event has a `kind` (one of `stuckLoAdvance`, `singleLoDeadlock`, `repeatedDemotions`, `sustainedLlmFailure`, `cascadeHalt`, `emptyObjectivesBlock`, `subgoalDeletedRedirect`) and `severity` (`strong` drives the dashboard badge until `acknowledged: true`; `audit` is visible-only). Since #165 every doc also carries `clientVersion` — the `kAppVersion` of the build that wrote it — so a laptop that has fallen behind, and whose `toMap`s therefore drop the newer fields on every write, is visible from the audit trail rather than inferred from the shape of its belief docs.

**`questions`** *(new, #185; partition `/subgoalId`)* — the question bank: one doc per question the tutor generated, doc id `${subgoalId}_${contentHash}` (a hash of type, code and options — plus the prompt when there are no options — so an identical generation lands on the same doc). Model: [services/question_bank/bank_question.dart](lib/services/question_bank/bank_question.dart); service: [question_bank_service.dart](lib/services/question_bank/question_bank_service.dart). Fields: `subgoalId`, `rootGoalId`, `targetLOIds`, `questionType` (`ChatRequestType` name), `difficulty`, `language`, `payload` (the META as the UI renders it, TEXT as `prompt`, MCQ `correct` as option text), `model`, `createdAt`, `createdByUid`, `askedCount` / `answeredCount` / `correctCount`, `lastAskedAt`, `optionFeedback[]` (`{option, text, quality}`, the first grader feedback per picked option), `keyDisputedCount` / `keyDisputedAt` (gradings that called the answer key wrong, #198 — with a grade against the key, what `graderDisagreesWithKey` flags), `status` (`active` | `hidden`), `teacherNote`, `reviewedAt`. Written best-effort and unawaited by `TutorService` (a question when it arrives, the answer when it is graded; `turn_history` docs carry `questionId`); a missing container is logged and left alone for 10 minutes. Read by the teacher's Questions page (`Section.questions`), and by the tutor to serve questions from the bank (#186, CONDUCTOR_POLICY §2.7): before a planned question is generated, `BankChoice` ([services/tutor/bank_choice.dart](lib/services/tutor/bank_choice.dart)) may take a fitting, active, unseen one instead — at least `QuestionBankMinimum` (8) of them and then with chance `QuestionBankShare` (0.5), both in `config/global`; a warm-up or recheck whenever one fits. A multiple-choice pick on a bank question is graded from its key (fixed signal on the target LO, feedback text from `optionFeedback`, one `mcqAnswer` call only when that text is missing); turn records carry `fromBank` / `gradedByKey`.
...
3. `GoalsApp` (`ConsumerWidget`) watches `authServiceProvider`, `accountServiceProvider`, and `localApiKeyStorageProvider`:
   ...
   - the config's first answer still pending → the same spinner; `config/global`'s `MinimumVersion` above this build → `UpdateRequiredScreen` (#165), with no shell and no session behind it.
...
2. On every tutor request, [InstructionGenerator.generateInstructions(type)](lib/services/tutor/instruction_generator.dart) reads instruction docs from `InstructionsService.cachedAll`, finds the doc whose id matches the `ChatRequestType` name, concatenates its sections, then assembles the prompt as `envelopeContract + alwaysInclude + typeSpecific` (deliberate order for prompt-cache prefix stability), then — on an `mcqAnswer` call only — the fixed `answerKeyDirective` (how to treat the `correct_option` a pick's grading input carries, #197), and last the output-language directive (#117).
...
5. The connector keeps three history lists capped at 50 entries each, and includes one of them per call based on `PreviousInputs.includeAll | includeSession | exercise | newSession` (see "Conversation history" in `docs/LLM_CONTRACT.md`). The streaming path only records the user turn into history once the stream finalises successfully and the parsed response isn't an `ErrorResponse`.
...
- **Cosmos DB** account with database `python-tutor` and containers:
  ...
  - per-subgoal (partition `/subgoalId`): `questions` (#185)
...
A **minimum version** (#165) closes the gap **Later** leaves open: a student who declines every update keeps writing with a build whose `toMap`s do not know the fields later builds added, and since Cosmos treats an upsert as a whole-document replacement, every such write erases them again (`highestPositiveDifficulty` vanished from restored belief docs within two minutes, written by a laptop still on a pre-#103 build). `config/global` may therefore carry `MinimumVersion`; [core/update_required.dart](lib/core/update_required.dart) evaluates it against `appVersionProvider` — unknown until the config's first answer, then re-evaluated on every poll — and `GoalsApp` mounts [UpdateRequiredScreen](lib/widgets/update_status.dart) instead of the shell for a build below it: the same check, download and checksum wiring, **Update** or **Check for updates**, and nothing to dismiss. A minimum raised during a lesson takes effect on every open app within one poll; one lowered again lets them back in. A blank or unparseable minimum is ignored (logged), so a typo cannot lock the school out. For detection, every `turn_history` doc carries `clientVersion` since #165.
